...
class ConfigReader:
    ...
    def __init__(self, config_file):
        ...
        if not os.path.exists(self.config_file):
            ...
            raise ValueError(f"警告: 設定ファイル {self.config_file} が見つかりません。")
        else:
    ...
    def get(self, section, option):
        ...
        if not self.config.has_section(section):
            ...
            raise ValueError(f"警告: セクション {section} が見つかりません。")
            return None

        # check option
        if not self.config.has_option(section, option):
            print(f"警告: オプション {option} が見つかりません。")
            raise ValueError(f"警告: オプション {option} が見つかりません。")
            return None
...
if __name__ == "__main__":
    ...
    config_reader = ConfigReader('InitConfig/config.ini')
    ...
    val_str = config_reader.get_list('tool', 'mail_draft')
    print(val_str)
